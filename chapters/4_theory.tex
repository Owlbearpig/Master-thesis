In this chapter we will introduce the basic physical principles which help understand how birefringent waveplates work. Simply put the purpose of a waveplate is to modify the polarization state of EM waves in a controlled way as they pass through the waveplate. This is most commonly achieved by making the plates out of a birefringent material with a certain thickness. To this end, the underlying physical principles birefringence and dichroism are explained in more detail in the sections \ref{sec:dichroism} and \ref{sec:birefringence}. Any change to a polarization state and the polarization states themselves, can be described mathematically using the Jones calculus which will be shown in section \ref{sec:math_desc}. Finally, this leads to the description of the waveplate design as it is used in this work as well as the associated optimization problem. Both are presented in section (\ref{sec:achromatic_waveplates}) of this chapter.

\section{The polarization ellipse}
\label{sec:polellipse}
There are several ways to describe the polarization state of EM-waves. One of them is the polarization ellipse which is a geometrical representation of the state. This remained the only satisfactory way to visualize the different polarization states for almost the entire 19\textsuperscript{th} century, although it is not very practical for carrying out calculations. At the end of the century another representation was proposed by Henri Poincaré, now known as the Poincaré sphere, which could be used for both calculations and visualization of the polarization states \cite{Collett2008a}. Poincaré's representation will be explained in section \ref{sec:the_poincare_sphere}. The polarization ellipse can be derived either directly from a solution to the EM-wave equation or starting from Maxwell's equations.  It is practical to start from Maxwell's equations, since they are used for derivations later on in this work and almost all equations used originate from his equations. In differential form Maxwell's equations are given by:
\par
...
\begin{figure}[h]
    \centering
    \includestandalone{4_chapter04/tikz_e_wave}
...
\begin{figure}[h]
    \centering
    \includestandalone[scale=1.1]{4_chapter04/tikz_linear_pol}
...
\centering
    \begin{subfigure}
        \centering
        \includestandalone[scale=0.8]{4_chapter04/tikz_circ_pol_lh}
    \end{subfigure}
    \begin{subfigure}
        \centering
        \includestandalone[scale=0.8]{4_chapter04/tikz_circ_pol_rh}
    \end{subfigure}
...
\begin{table}[h]
    \centering
    \includestandalone{4_chapter04/pol_summary_table}
...
\begin{figure}[h]
    \centering
    \includestandalone[scale=1.1]{4_chapter04/tikz_pol_ellipse}
...
\begin{figure}[h]
    \centering
    \includestandalone[scale=1.3]{4_chapter04/tikz_phasor_rotation}
...
\begin{figure}[h]
    \centering
    \includestandalone[scale=1.30]{4_chapter04/tikz_wire_grid_polarizer}
...
\begin{figure}[h]
    \centering
    \includestandalone[scale=0.9]{4_chapter04/tikz_electron_shell}
...
\begin{figure}[h]
    \centering
    \includestandalone[scale=0.95]{4_chapter04/tikz_calcite_beams}
...
\begin{figure}[h]
    \centering
    \includestandalone[scale=0.73]{4_chapter04/tikz_uniaxial_source}
...
\begin{figure}[h]
    \centering
    \includestandalone[scale=1.5]{4_chapter04/tikz_rytov_derivation}
...
\begin{figure}[h]
    \centering
    \includestandalone[scale=2]{4_chapter04/tikz_stratified_structure}
...
\begin{figure}[h]
    \centering
    \includestandalone[scale=1]{4_chapter04/tikz_index_ellipse}
...
\begin{figure}[h]
    \centering
    \includestandalone[scale=2.7]{4_chapter04/tikz_interface_derivation}
...
\begin{figure}[h]
    \centering
    \includestandalone[scale=1.3]{4_chapter04/tikz_waveplate_conversions}
...
\begin{figure}[h]
    \centering
    \includestandalone[scale=1.63]{4_chapter04/tikz_half_waveplate}
...
\begin{figure}[h]
    \centering
    \includestandalone[scale=0.9]{4_chapter04/tikz_poincare_sphere_intro}
...
\begin{figure}[h]
    \centering
    \includestandalone[scale=1]{4_chapter04/tikz_frame_rotation}
...
\begin{table}[H]
    \centering
    \includestandalone{4_chapter04/pol_statevectors_table}
...
\begin{figure}[h]
    \centering
    \includestandalone[scale=1]{4_chapter04/tikz_horizontal_diattenuator}
...
\begin{table}[h]
    \centering
    \includestandalone{4_chapter04/masson_result_table}
...
A similar AWP is presented in another work by Wu, et al. Here they use sapphire instead of quartz which reduces the overall thickness of the AWP to \SI{2.453}{\milli \meter} since compared to quarts sapphire has a higher birefringence. They report a value of 3.39 and 3.07 for the ordinary and extraordinary refractive index respectively. Similarly, they optimize the loss function given in equation \ref{eq:mass_loss} and by stacking up to 20 individual plates they obtain a deviation less than \SI{0.5}{\percent} from the $\pi/2$ target over the frequency range \SIrange{0.2}{2.0}{\tera \hertz}. Table \ref{tab:wu_result} shows their obtained values for the result with an overall thickness of \SI{2.453}{\milli \meter} consisting of six plates. This design was produced and characterized but showed more than a threefold increase in phase deviation compared to the optimization result. The deviation was attributed to angle misalignment and a mismatch between the used values from the actual values of the index of refraction of sapphire \cite{Wu2020}. 

\begin{table}[h]
    \centering
    \includestandalone{4_chapter04/wu_result_table}
...
\end{equation}
with $r=\frac{T_{1,1}}{T_{2,1}}$ and $\bm{\psi}=\psi_1, ..., \psi_n$, $\bm{d}=d_1, ..., d_n$, etc. In other words we optimize the set of parameters so that $\bm{\mathcal{E}}_o=\bm{\mathcal{E}}_{RCP}$.
...
% TODO for results?. Give explanation why two l4 dont give l2. Problem is l4 is also rotator (is it?) -> different azimuths -> different outputs since l4 is rotator depending on azimuth ... 
% only for 100% pure circ is azimuth undef. (never happens)

% TODO write summary + transition to chapter 4, right?
