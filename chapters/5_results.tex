...
We calculated the average values of $a$ and $b$ for the 10 different positions and obtained the following: $a=\SI[separate-uncertainty = true]{734.55(4560)}{\micro \meter}$, $b=\SI[separate-uncertainty = true]{392.95(3820)}{\micro \meter}$ and $d=\SI[separate-uncertainty = true]{1139.00(3360)}{\micro \meter}$ while the expected values are $a=\SI{600}{\micro \meter}$, $b=\SI{500}{\micro \meter}$ and $d=\SI{1100}{\micro \meter}$. Using these values of $a$ and $b$ together with the measured optical parameters of HIPS we calculated the new adjusted form birefringence for the average stratification.
The result of this is shown in figure \ref{fig:ri_stripe_err} shows the calculated refractive indices and the averaged $a$ and $b$ as well as the design ($a$, $b$) for the parallel $(n_p)$ and perpendicular $(n_s)$ direction relative to the stratification. We see that the refractive indices in both directions are approximately $0.05$ smaller for the design values of $a$ and $b$ compared to the measured values. However the birefringence is around $0.01$ larger for the values of the design.
...
Since we had found most other possibilities to be unlikely the last option was that there was a problem determining the birefringence. We therefore compared the calculated birefringence to a CST simulation\footnote{CST Studio Suite is a commercial software for analyzing electromagnetic components using the finite element method. A few additional remarks to the simulation in the appendix section \ref{sec:CST simulation}}. 


We printed test grating ...
...
% TODO theory chapter intermission rewrite, no hermann waveplate.
\begin{itemize}
    ...
    \item add single waveplate to result plots for comparison like masson (replace csv file)
