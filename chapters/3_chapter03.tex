...
\end{split}
\end{align}
where $\alpha_j = k \sqrt{\epsilon_j-n^2}$ and $j$ is 1 $\text{if $z \bmod d \in [0, a)$}$ else $2$. The field components parallel to the layers must be continuous at the interfaces and therefore also periodic. I.e. because of equation \ref{eq:rytov_field_ansatz} the amplitudes at $\pm 0$ and $a, -b$ are equal:
\begin{align}
\begin{split}
    &U(+0) = U(-0), 
    \qquad
...
\end{split}
\end{align}
We substitute the amplitudes (equation \ref{eq:rytov_amplitudes}) in these boundary conditions so that we get four equations for the coefficients $A_1,A_2,B_1$ and $B_2$:
\begin{align}
\begin{split}
    A_2 = A_1, 
    \qquad
...
\end{equation}
%At this point we are ready to calculate the mean values of the fields. 
