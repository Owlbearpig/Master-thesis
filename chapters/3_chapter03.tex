...
\end{equation}
where $t$ denotes the field components and paths tangential to both sides of the interface. Since  we did not choose any specific interface or electric field we can omit the integration. This gives us the continuity condition for the tangential components of the electric field at the interface:
\begin{equation}
    \bm{n} \times (\bm{E_1} - \bm{E_2}) = 0,
\end{equation}
where $\bm{n}$ is normal to the interface, $\bm{E_1}$ and $\bm{E_2}$ are the electric fields in medium one and two respectively. Again, we can apply the same procedure for the magnetic field. Here instead we use Gauss's law for magnetism $(\bm{\nabla}\cdot \bm{B} = 0)$. Integrating over a volume enclosing the interface and then shrinking the width of this volume gives us a condition for the magnetic field at the boundary:
\begin{equation}
    \bm{n}\cdot(\bm{B_1}-\bm{B_2}) = 0,
\end{equation}
likewise $\bm{B_1}$ and $\bm{B_2}$ are the magnetic fields in medium one and two respectively. In other words the normal component of the magnetic field is continuous across the interface \cite{Griffiths2017IntroductionElectrodynamics}.

\newpage

Specifically, in the context of this work we are interested in the reflection and transmission coefficients of the interface between two periodically stratified media 1 and 2. In the previous section $\ref{sec:form_birefringence}$ about form birefringence we derived approximations for the dielectric tensors of such periodic media given the individual layer material and thickness. Additionally, in the case we consider the two anisotropic media on each side of the interface are rotated at some angles $\alpha_1$ and $\alpha_2$ around a common x-axis. This means that in a third system the dielectric tensors of both media given by equation \ref{eq:rytov_tensor} will not be diagonal. We could use the principal coordinate system of one media but since there in general are multiple interfaces we instead use a lab system as the reference system. In the lab system $\hat{\epsilon}$ is given by:
\begin{equation}
    \hat{\epsilon_i}(\alpha_i) = R_x(\alpha_i) \hat{\epsilon_i}' R_x(-\alpha_i),
\end{equation}
where $\hat{\epsilon_i}'$ is the dielectric tensor in its principal coordinate system, $i=1,2$ and $R_x(\alpha_i)$ rotates a vector around the x-axis by an angle $\alpha_i$. $R_x(\alpha_i)$ can be represented as:
\begin{equation}
    ...
    \end{pmatrix}.
\end{equation}
With this we can proceed with the derivation of the reflection and transmission coefficients. Respectively, we can represent the incident, transmitted and reflected wave as follows:
\begin{align}
\begin{split}
    \bm{E_i} = \bm{E_{0,i}}e^{i(\bm{k_i}\bm{r}-\omega_i t)},
    \quad
    \bm{E_t} = \bm{E_{0,t}}e^{i(\bm{k_t}\bm{r}-\omega_t t)},
    \quad
    \bm{E_r} = \bm{E_{0,r}}e^{i(\bm{k_r}\bm{r}-\omega_r t)}.
\end{split}
\end{align}
Following???, from the boundary conditions it follows that: 
