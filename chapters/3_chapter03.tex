...
\end{equation}
which in the context of this work should always be equal to one. In the same sense we can define the degree of linear polarization $V_l$ and the degree of circular polarization $V_c$ as:
\begin{equation}
    V_l = \frac{\sqrt{s_1^2+s_2^2}}{s_0} = V \cos 2\chi \qquad V_c = \frac{s_3^2}{s_0} = V \sin 2\chi,
\end{equation}
where the angle $\chi$ is the orientation angle in the polarization ellipse representation. $V_l$ and $V_c$ are measures of how much of the intensity can be allocated to the linear and circular polarization components respectively. Since $\chi$ is in the range of $\pm \frac{\pi}{4}$, $V_l$ is strictly positive. Meanwhile, in this range $V_c$ can be negative where for positive values the light is RCP and for negative values of $V_c$ the light is LCP.

\cite{HechtOpticsEdition, Shurcliff1962PolarizedLight, GilPerez2017PolarizedApproach}
