...
\begin{equation}
    ...
    \end{pmatrix},
\end{equation}
which is a \SI{45}{\degree} linear polarized state. The scalar product of two Jones vectors $\bm{\tilde{u}}$ and $\bm{\tilde{v}}$ is defined by:
\begin{equation}
...
So far we have seen two mutually complementary descriptions of the polarization states. Both have advantages and disadvantages but since in the context of this work the light is fully polarized and coherent the Jones vectors are best suited for . Following we will describe the representation of waveplates in the Jones and Stokes formalisms.
\cite{HechtOpticsEdition, GilPerez2017PolarizedApproach}

\begin{table}[h]
    \centering
    \includestandalone{3_chapter03/pol_statevectors_table}
    \caption{Summary of the Jones and Stokes vectors for the degenerate polarization states. All vectors are normalized. }
