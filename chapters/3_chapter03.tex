...
% TODO2 section title :) (or Mathematical description of nondepolarizing media or mathematical formalism or something)
...
\end{equation}
If we further define the transmittance as the ratio between the transmitted intensity and the incident intensity, then we see that the states $\bm{\eta}_1$ and $\bm{\eta}_2$ are exactly the states with the highest and lowest transmittance respectively. In other words, with respect to any other normalized output state, the states $\bm{\eta}'_1$ and $\bm{\eta}'_2$ are the output states with the highest and lowest intensities respectively. 

\newpage

The last property we will introduce before applying these to define the waveplates mathematically in the Jones formalism is the normality of Jones matrices. This property is important for the calculation of the phase shift given a Jones matrix. Jones matrices can be separated into two groups depending on if their eigenstates are mutually orthogonal or not. Where the eigenstates are the states for which the Jones matrix only scales the state by the respective eigenvalue. Specifically, if the eigenstates are mutually orthogonal then the Jones matrix is said to be normal or homogeneous and nonnormal or inhomogeneous if they are nonorthogonal. 
