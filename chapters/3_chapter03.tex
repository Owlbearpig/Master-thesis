...
As we have seen already waveplates are highly wavelength sensitive due to the inverse wavelength dependency of the phase shift but also due to dispersion of the refractive index and consequently dispersion of the birefringence. In other words the three parameters $\Delta$, $p_1$ and $p_2$ of the four parameters which characterize a waveplate are wavelength dependent. There are several types of achromatic waveplates based on different techniques which can in theory compensate for this wavelength dependency. One possibility is to use the phase shift induced by total internal reflection which can be realized through a Fresnel Rhomb. Using this method it is possible to create an almost perfect achromatic retarder. The Fresnel Rhomb works over a large frequency range since the variation of retardance is purely due to variation of the refractive index with wavelength \cite{HechtOpticsEdition}. They often cause strong spatial shifts of the light, take up a lot of space and are hard to align since the retardation strongly depends on the angle \cite{Masson2006TerahertzJean-Baptiste}. Another method is to combine waveplates of different materials 
