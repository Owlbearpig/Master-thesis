...
\begin{equation}
    V_l = \frac{\sqrt{s_1^2+s_2^2}}{s_0} = V \cos 2\chi \qquad V_c = \frac{s_3^2}{s_0} = V \sin 2\chi,
\end{equation}
where the angle $\chi$ is the ellipticity angle in the polarization ellipse representation. $V_l$ and $V_c$ are measures of how much of the intensity can be allocated to the linear and circular polarization components respectively. Since $\chi$ is in the range of $\pm \frac{\pi}{4}$, $V_l$ is strictly positive. Meanwhile, in this range $V_c$ can be negative where for positive values the light is RCP and for negative values of $V_c$ the light is LCP. The values $V_c$ and $V_l$ are useful quantifiers of the polarization state. Before we continue on a similar note with the description of the Jones vectors, it is worth mentioning the vector representation of the Stokes parameters since it is widely used. Specifically, the Stokes parameters are usually arranged as a 4x1 Stokes vector $\bm{s}$:
...
\begin{equation}
    ...
    \end{pmatrix},
\end{equation}
where $E_x(t)$ and $E_y(t)$ are the scalar field components of the electric field. The Jones vector is, unlike the Stokes vector, an actual vector. Additionally, it is defined up to a non-measurable global phase factor $e^{i\phi}$. If we leave aside this global phase factor we can express the Jones vector in the polarization ellipse representation using the intensity $I$, the orientation angle $\psi$ and the ellipticity angle $\chi$ as follows:
