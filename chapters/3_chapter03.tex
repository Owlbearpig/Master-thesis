...
\begin{figure}[h]
    ...
    \caption{This figure shows a wire grid polarizer which is essentially a conducting sub-wavelength grating. From the left unpolarized light interacts with the wire grid so that field components parallel to the wires are eliminated. After passing the polarizer the light is therefore linearly polarized perpendicular to the wires.}
...
The explanation of the underlying principle is similar to that of dichroism and can be explained by considering the structure of the material. We can represent the binding of the electron cloud to the nuclei at each crystal lattice point using a mechanical model. The model consists of a shell bound to the nuclei with springs. Furthermore the springs have different spring constants and the nuclei is fixed in space. 

\begin{figure}[h]
    \centering
    \includestandalone[scale=1.00]{3_chapter03/tikz_electron_shell}
    \caption{Model of an electron cloud bound to its nucleus via springs of different strength. The nucleus is fixed in space. This model is a simplification of an optical anisotropic material, a model closer to reality would require a quantum mechanical description of the system.}
