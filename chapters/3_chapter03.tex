...
\begin{equation}
    ...
    \end{pmatrix},
\end{equation}
where according to equation \ref{eq:wp_eq} the $\Delta_i$ are proportional to the frequency and the thickness $d_i$ of the i-th waveplate. From the previous section we know that $\hat{T}_M$ can written as the product of a diattenuator and a retarder. Comparing $\hat{T}_{M}$ to $\hat{T}_{RL}(\psi, \Delta)$ we therefore get the following expression for $\Delta$:
\begin{equation}
    \tan^2 \frac{\Delta}{2} = \frac{\operatorname{Im}A^2+\operatorname{Im}B^2}
    {\operatorname{Re}A^2+\operatorname{Re}B^2}.
\end{equation}
The problem of determining the parameters $\psi_i$ and $d_i$ of the waveplates can be formulated as minimizing the following error function:
\begin{equation}
    L(\psi_i, d_i)=\sum_{\nu}\left(\Delta(\nu)-\frac{\pi}{2}\right)^2.
\end{equation}
The result of the optimization is shown in table 
...
- Are Pancharatnam plates the predecessor to these?
- Design here based on masson which is based on the design of the TAQ wave plate is based on the idea from Destriau and Prouteau.
- These older types are for visible range
- The key feature
