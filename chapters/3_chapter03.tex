...
\end{equation}
where we assume that the transmittance $p_2$ is not zero since then $\hat{T}$ represents a polarizer and therefore would have a determinant of zero. Furthermore, this equation is only valid for homogeneous Jones matrices. In the inhomogeneous case the following expression has to be used instead:
\begin{equation}
    \Delta = 2\cos^{-1}
\end{equation}
